  ...
  <q-dialog full-width full-height v-model="show">
    <q-card square>
      <q-bar class="bg-teal-14 text-white">
        <span style="font-size: 12px">Les articles supprimés({{article.meta.totalItems}})</span>
        <q-space />
        <q-btn v-close-popup flat dense icon="close" />
      </q-bar>
      <q-card-section style="height: calc(100% - 90px)" class="overflow-auto">
        <q-list v-if="article.meta.totalPages">
        ...
        <NoTrashData :total-items="article.meta.totalItems" :loading="loading" />
      </q-card-section>
      <q-separator />
